...
X_man['食堂消费占比'] = pd.cut(X_man['食堂消费占比'], [-0.1, 0.7, 0.85, 0.95, 0.99, 1.1], labels=[-2, -1, 0, 1, 2])
X_woman['食堂消费占比'] = pd.cut(X_woman['食堂消费占比'], [-0.1, 0.7, 0.85, 0.95, 0.99, 1.1], labels=[-2, -1, 0, 1, 2])

label_list = ['很高(2)', '高(1)', '中(0)', '低(-1)', '很低(-2)']
num_list1 = [len(X_man[X_man['食堂消费占比'] == i]) for i in [2, 1, 0, -1, -2]]
num_list2 = [len(X_woman[X_woman['食堂消费占比'] == i]) for i in [2, 1, 0, -1, -2]]
...
plt.xlabel('食堂消费占比离散值', fontproperties=font)
