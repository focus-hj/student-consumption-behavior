...
X_man['消费总额'] = pd.cut(X_man['消费总额'], [0, 80, 200, 380, 500, 1000], labels=[-2, -1, 0, 1, 2])
X_woman['消费总额'] = pd.cut(X_woman['消费总额'], [0, 90, 200, 315, 430, 1000], labels=[-2, -1, 0, 1, 2])

label_list = ['很高(2)', '高(1)', '中(0)', '低(-1)', '很低(-2)']
num_list1 = [len(X_man[X_man['消费总额'] == i]) for i in [2, 1, 0, -1, -2]]
num_list2 = [len(X_woman[X_woman['消费总额'] == i]) for i in [2, 1, 0, -1, -2]]
...
plt.xlabel('消费额离散值', fontproperties=font)
